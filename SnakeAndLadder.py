...
class Player:
    def __init__(self, name, color, pos, is_computer=False):
        self.name = name
        self.color = color
...
class SnakeLadderGame:
    ...
    def setup_game(self):
        ...
        if self.game_mode == "pvp":
            for i in range(self.num_players):
                ...
                player = Player(f"Player {i+1}", colors[i], (start_x + offset, start_y), is_computer=False)
                self.players.append(player)
        else:  # pvc mode
            # First player is human
            player = Player("Player 1", colors[0], (start_x - 10, start_y), is_computer=False)
            self.players.append(player)
            # Rest are computer players
            for i in range(1, self.num_players):
                offset = (i - self.num_players / 2 + 0.5) * 20
                computer = Player(f"Computer {i}", colors[i], (start_x + offset, start_y), is_computer=True)
    ...
    def handle_mode_select_click(self, pos):
        # Player vs Player button
        if 150 < pos[0] < 325 and 350 < pos[1] < 450:
            self.game_mode = "pvp"
            self.setup_game()
        # Player vs Computer button
        elif 375 < pos[0] < 550 and 350 < pos[1] < 450:
            self.game_mode = "pvc"
            self.setup_game()
    ...
    def handle_roll_click(self, pos):
        ...
        # Check if clicked near dice and it's not computer's turn
        current = self.players[self.current_player]
        if dist < 50 and not self.dice.rolling and not current.is_computer:
            self.dice.roll()
        ...
        if self.state == GameState.PLAYING:
            ...
            if current.is_computer:
                if self.computer_delay <= 0:
                    self.dice.roll()
                    self.state = GameState.ROLLING
                    self.computer_delay = 60  # Reset delay for next computer turn
                else:
            ...
            if player.update():  # If reached destination
                ...
                if encounter == "snake":
                    ...
                    if player.position == 100:
                        ...
                        self.state = GameState.PLAYING
                        self.computer_delay = 60  # Set delay for next turn
    ...
    def draw_mode_select(self):
        ...
        # Title
        title = self.large_font.render("SELECT MODE", True, YELLOW)
        self.screen.blit(title, (WIDTH//2 - title.get_width()//2, 100))
        
        # Subtitle
        subtitle = self.small_font.render(f"{self.num_players} Players Game", True, WHITE)
        self.screen.blit(subtitle, (WIDTH//2 - subtitle.get_width()//2, 200))
        
        # Mode buttons
        # Player vs Player
        pvp_rect = pygame.Rect(150, 350, 175, 100)
        pygame.draw.rect(self.screen, ORANGE, pvp_rect, border_radius=15)
        pygame.draw.rect(self.screen, WHITE, pvp_rect, 4, border_radius=15)
        
        pvp_label1 = self.font.render("Player", True, WHITE)
        pvp_label2 = self.font.render("vs", True, WHITE)
        pvp_label3 = self.font.render("Player", True, WHITE)
        self.screen.blit(pvp_label1, (pvp_rect.centerx - pvp_label1.get_width()//2, pvp_rect.y + 15))
        self.screen.blit(pvp_label2, (pvp_rect.centerx - pvp_label2.get_width()//2, pvp_rect.y + 45))
        self.screen.blit(pvp_label3, (pvp_rect.centerx - pvp_label3.get_width()//2, pvp_rect.y + 70))
        
        # Player vs Computer
        pvc_rect = pygame.Rect(375, 350, 175, 100)
        pygame.draw.rect(self.screen, CYAN, pvc_rect, border_radius=15)
        pygame.draw.rect(self.screen, WHITE, pvc_rect, 4, border_radius=15)
        
        pvc_label1 = self.font.render("Player", True, WHITE)
        pvc_label2 = self.font.render("vs", True, WHITE)
        pvc_label3 = self.font.render("Computer", True, WHITE)
        self.screen.blit(pvc_label1, (pvc_rect.centerx - pvc_label1.get_width()//2, pvc_rect.y + 15))
        self.screen.blit(pvc_label2, (pvc_rect.centerx - pvc_label2.get_width()//2, pvc_rect.y + 45))
        self.screen.blit(pvc_label3, (pvc_rect.centerx - pvc_label3.get_width()//2, pvc_rect.y + 70))
        
        # Footer
        footer = self.small_font.render("Press ESC to go back", True, (200, 200, 200))
        self.screen.blit(footer, (WIDTH//2 - footer.get_width()//2, HEIGHT - 50))
    ...
    def draw_game(self):
        ...
        if self.state == GameState.PLAYING and not self.dice.rolling:
            current = self.players[self.current_player]
            if not current.is_computer:
                roll_text = self.small_font.render("Click Dice!", True, WHITE)
                self.screen.blit(roll_text, (WIDTH - 120, 100))
            else:
                roll_text = self.small_font.render("Computer Turn", True, WHITE)
                self.screen.blit(roll_text, (WIDTH - 140, 100))
